...
[[noreturn]] inline void fail(nn::Location location, const std::string& message) {
...
inline std::string make_message(const char* expression, const std::string& message) {
  std::ostringstream oss;
...
#else
#define NN_ASSERT(condition) \
  do {                       \
    (void)sizeof(condition); \
  } while (0)
#define NN_ASSERT_MESSAGE(condition, message) \
  do {                                        \
    (void)sizeof(condition);                  \
  } while (0)
#endif