...
std::pair<std::any, Matrix> NonLinLayer::forward(const Matrix& input) const {
  Matrix output = func_(input);
  NonLinState state{input, output};
  return {std::any(std::move(state)), std::move(output)};
}

std::pair<std::any, Matrix> NonLinLayer::backward(const std::any& state,
  ...
  Matrix grad_input(grad_output.rows(), grad_output.columns());
